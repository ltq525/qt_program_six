...
DFA::DFA()
{
    memset(this, 0, sizeof(DFA));
}

int DFA::change(int Color)
{
    if ( Color == 1 )
...
int DFA::check(move_t bestMove[], move_t now)
{
    int i;
    for (i=0; i<count; i++){
...
void DFA::new_match2(pos_t point, dfa_t *pdfa, move_t bestMove[], int direction)
{
    int p=0, j, step;
...
void DFA::new_match(pos_t point, move_t bestMove[], int ori_direction)
{
    int i, direction;
    pos_t temp;
...
void DFA::addpoint(move_t bestMove[], pos_t point)
{
    int i, j, k;
    for (i=1; i<=19; i++){
        for (j=1; j<=19; j++){
...
// Match function.
void DFA::match2(pos_t point, dfa_t *pdfa, move_t bestMove[], int direction, int dfa_num)
{
    int p=0, j;
...
void DFA::match(pos_t point, int direction, move_t * bestMove)
{
    int i;
    pos_t temp;
    for (i=0; i<m_dfa_index; i++){
...
int DFA::TypeMatch(char ourColor, move_t bestMove[], char board[][GRID_NUM])
{
    int i, j;
    //int temp_count;
    pos_t point;
 ...
 * (1,{}) -------> (2,{}) -------> (3,{}) -------> (4,{}) ------> (5,{2210})
 */
bool DFA::DfaCreat(dfa_t *pdfa, char str[])
{
    ...
    for (i=0; str[i]!='\0' && strchr("$#+-|OoXx.?,!a*", *str); i++){
        ...
        if (new_state >= pdfa->max_states)
            DfaResize(pdfa, pdfa->max_states + Size, pdfa->max_indexes + Size);
    }
...
void DFA::DfaResize(dfa_t *pdfa, int max_states, int max_indexes)
{
  state_t *pBuf;
  attrib_t *pBuf2;
  int i;
...
bool DFA::dfa_init()
{
    ...
    for (int i =0; i < max_index; i++){
        ...
        DfaResize(&m_dfa_array[m_dfa_index], Size, Size);

        // Create the DFA.
        if ( DfaCreat(&m_dfa_array[m_dfa_index], input) )
            m_dfa_index++;
    }
    fclose(m_partin);
    // printf("XXXX %d\n", m_dfa_index);

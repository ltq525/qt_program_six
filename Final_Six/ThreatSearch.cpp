...
int ThreatSearch::init() {
    return m_dfa.dfa_init();
}

void ThreatSearch::init_game() {
    m_has_win = 0;
}

void ThreatSearch::BeforeSearch(char board[][GRID_NUM], char color){
    memcpy(m_board, board, sizeof(m_board));
    m_chess_type = color;
    m_vcf_node = 0;
}

bool ThreatSearch::ThreatJudge(move_t * preMove)
{
...
int ThreatSearch::is_attack(char board[GRID_NUM][GRID_NUM],char Color, move_t * Move)
{
    int x, y, newx, newy;
    int count;
    int i, d, j;
...
int ThreatSearch::is_dlb_attack(char board[][GRID_NUM],char Color, move_t * Move)
{
    int x, y, newx, newy;
    int count;
    int i, d, j;
    ...
    for ( i = 0; i <= 1; i++ )
    {
        ...
        for ( d = 0; d < 4; d++  )                  // 四个方向判断
        {
            ...
            if (tmpboard[rightx][righty] == NOSTONE) // NOSTONE.
            {
                ...
                {
                    //连上四个，但两端之一是敌子或者边界则没有进攻意义，因为人家一个子可以抵消你两个子的影响
...
int ThreatSearch::is_three(char position[GRID_NUM][GRID_NUM],char Color, pos_t * Pos)
{
    int x, y, newx, newy;
    int count;
    int  d, j;
...
int ThreatSearch::GetMoveList( char ourColor,char a_d, pos_t * canUse, int n_Pos, move_t * moveList, move_t * preMove)
{
    ...
    {
        // Siran Lin's
        n_MoveList=m_dfa.TypeMatch(ourColor,moveList, tmpboard);
...
inline int ThreatSearch::vcf_abs(int a)
{
    return a >= 0 ? a : -a;
}

inline int ThreatSearch::dist(pos_t p1, pos_t p2, pos_t pt)
{
    return (vcf_abs(p1.x-pt.x)+vcf_abs(p1.y-pt.y)+vcf_abs(p2.x-pt.x)+vcf_abs(p2.y-pt.y));
}

inline int ThreatSearch::vcf_min(int a, int b)
{
    return a <= b ? a : b;
}
...
void ThreatSearch::sort(move_t * moveList, int n_moveList, move_t * preMove)
{
    int i;
...
unsigned long ThreatSearch::vcf_hash_board(char board[GRID_NUM][GRID_NUM])
{
    unsigned long ans=0;
    int i, j;
    for (i=1;i<=19;i++)
...
int ThreatSearch::vcf_hash_check(HashNode node)
{
    int i, t;
    int dep=node.dep;
...
bool ThreatSearch::anti_vcf_search(int depth,char ourColor,move_t * bestMove,move_t * preMove, int preNode, int prePos)
{
    int Color = ourColor;
    int i,j, t;
    ...
    if (is_win_by_premove(m_board, preMove))        // 如果已经赢了，不再搜索
    {
    ...
    {
        n_moveList=GetMoveList(Color,1,canUse,n_pos,moveList, preMove);   // Get the move list.
    ...
    {
        n_moveList=GetMoveList(Color,1,canUse,n_pos,moveList, preMove); //生成招法
    ...
    else
    {
        ...
        n_moveList=GetMoveList(Color,0,canUse,n_pos,moveList, preMove);
        j=m_vcf_total_node;
        for (i=0;i<n_moveList;i++)
        {
            NodeID=++m_vcf_total_node;
...
bool ThreatSearch::vcf_search(int depth,char ourColor,move_t * bestMove,move_t * preMove, int preNode, int prePos)
{
    int Color = ourColor;
    int i,j, t;
    ...
    if (Color==(m_chess_type) && is_attack(m_board,Color^3,preMove))
    {
        ...
        n_moveList=GetMoveList(Color,1,canUse,n_pos,moveList, preMove);
